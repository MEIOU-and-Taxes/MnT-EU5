...
The dispatcher is index-agnostic: a call site passes `DATA_SET` (one of `market`, `location`, `human_location`) and a `CYCLE`. The accessor, count global, and per-(data set, cycle) cache variables are all constructed from `DATA_SET` by naming convention, so no constants or branches are needed internally. Identical `(DATA_SET, CYCLE)` instantiations share cached chunk sizes automatically; sharing is by textual spelling of `CYCLE`, so a literal and a script value that evaluate to the same number hold separate caches. Any dense global index with a matching accessor and count global can ride the dispatcher. Up to 512 countries work each pass.
...
`mnt_wealth_market_index` (global map): every market in the world. Rebuilt whole each month because markets are created and destroyed during play; the set is small (a few hundred), so the rebuild is cheap. Swept through `mnt_wealth_market_index_accessor` with `mnt_wealth_market_count`.

Market passes ride the balancer with `DATA_SET = market`, `CYCLE = 1`: every market is processed the same day, spread across the workers. The demand aggregations (`cost_of_the_court.2` dispatching `apply_cost_of_the_court_demand` and `diplomatic_spending.2` dispatching `apply_diplomatic_spending_demand`; workers in `MnT_market_demand_workers.txt`) are identical `(market, 1)` instantiations that share one cached chunk size. They run one day after the monthly pulse, when every country's accumulation is already on the market centers.
